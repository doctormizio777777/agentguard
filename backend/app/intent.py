...
def judge_intent(
    ...
    *,
    ...
    provider = client or _build_client()
    messages = [
    ...
    for attempt in range(2):
        try:
            response = _request_verdict(provider, model, messages)
...
def _request_verdict(client: Any, model: str, messages: list[dict[str, str]]) -> Any:
    try:
